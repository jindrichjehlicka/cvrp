...
def extract_final_scores(df):
    ...
    for _, row in df.iterrows():
        key = (row['max_iterations'], row['population_size'], row['generations'], row['initial_temperature'],
               row['cooling_rate'])
...
def group_by_parameter(final_scores):
    grouped = final_scores.groupby(
        ['max_iterations', 'population_size', 'generations', 'initial_temperature', 'cooling_rate']).agg(
...
def perform_anova(grouped):
    ...
    for max_iter in grouped['max_iterations'].unique():
        groups = [group['mean_final_score'].values for name, group in
                  grouped[grouped['max_iterations'] == max_iter].groupby('population_size')]
        if len(groups) > 1:
            anova_results[f'max_iterations_{max_iter}'] = f_oneway(*groups)

    # Group by population_size
    for population in grouped['population_size'].unique():
        groups = [group['mean_final_score'].values for name, group in
                  grouped[grouped['population_size'] == population].groupby('generations')]
        if len(groups) > 1:
            anova_results[f'population_size_{population}'] = f_oneway(*groups)

    # Group by generations
    for generation in grouped['generations'].unique():
        groups = [group['mean_final_score'].values for name, group in
                  grouped[grouped['generations'] == generation].groupby('initial_temperature')]
        if len(groups) > 1:
            anova_results[f'generations_{generation}'] = f_oneway(*groups)
...
if __name__ == "__main__":
    file_pattern = '../parameter_testing/ga_sa/ga_sa_hybrid_epoch_data_chunk_*.csv'
    main(file_pattern)
