...
def run_and_collect_data(instance_name, depot_loc, node_loc, demand, capacity, optimal_cost, runs=20):
    ...
    for run in range(runs):
        ...
        routes, total_cost = dijkstra_cvrp(depot_loc, node_loc, demand, capacity)
        end_time = time.time()
        elapsed_time = end_time - start_time

        cost_difference = total_cost - optimal_cost

        instance_results.append(['Dijkstra', instance_name, run + 1, 'NA', cost_difference, elapsed_time, 'NA'])
...
results.to_csv('greedy_performance.csv', index=False)
