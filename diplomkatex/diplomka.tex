...
%    kapitoly)}\label{praktickuxe1-ux10duxe1stempirickuxe1-ux10duxe1stvlastnuxed-pruxe1ce-nenuxed-nuxe1zev-kapitoly}}
    ...
    \paragraph{Grid Search Method}
    In this study, we use a grid search method to find the best parameters for our algorithms. The grid search tests all possible combinations of parameters we provide. We use a tool from the \texttt{scikit-learn} library called \texttt{GridSearchCV} to do this.

    The grid search uses a method called cross-validation to check how well the parameters work. Here, we use 3-fold cross-validation. This means we split our data into three equal parts, called folds. In each test, two parts are used to train the algorithm, and one part is used to test it. This process is repeated three times, each time using a different part as the test set. This way, every part of the data is used for testing once.

    This method ensures that the algorithm is tested on different parts of the data, making sure the results are reliable and not just good for one specific set of data.

    The \texttt{GridSearchCV} tool checks each combination of parameters multiple times, as specified by the number of cross-validation folds. This means each parameter set is tested on different splits of the data, providing a thorough evaluation of how the parameters affect the algorithm's performance.

    By using grid search and cross-validation, we can find the best parameters for our algorithms. This process helps ensure that the algorithms perform well on different problem instances, making them more robust and reliable.
